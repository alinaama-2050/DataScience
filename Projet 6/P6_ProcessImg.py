...
if __name__ == "__main__":
    ...
    img_name = "shade"
    txt_name = "shade"
    img_path = f"data/Train/shade/{img_name}.jpg"
    anno_apth = f"data/Train/shade/{txt_name}.txt"
    ...
    # Image Enhanced
    enhanced_img = cv2.detailEnhance(image)
    enhanced_img = cv2.edgePreservingFilter(image, flags=1, sigma_s=64, sigma_r=0.2)
    cv2.imwrite(f"{dir_path}/{img_name}_enhanced.jpg", enhanced_img)
    ...
    chg_img = cv2.resize(image, (int(width), int(height)))
    cv2.imwrite(f"{dir_path}/{img_name}_resized.jpg", chg_img)

    ###Distort the image
    dist_img = cv2.flip(image, 1)  # Flip horizontally
    dist_img = cv2.flip(image, 0)  # Invert vertically
    cv2.imwrite(f"{dir_path}/{img_name}_distort.jpg", dist_img)
